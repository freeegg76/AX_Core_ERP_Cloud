...
- **`service_role` 키는 브라우저·프론트 레포 어디에도 두지 않는다.** CI 의 하드 가드가 강제한다([§18.3](#183-cicd)).
- **키 형식이 두 가지 운용된다.** 구형은 JWT(`eyJ…`, 페이로드의 `role` 로 구분), 신형은 불투명 문자열(`sb_publishable_…` = anon, `sb_secret_…` = service_role). **신형은 디코드할 수 없으므로 접두어가 유일한 단서**이며, JWT 파싱만 하는 검사기는 신형 anon 키를 판별하지 못한다. `scripts/check-no-service-role.mjs` 와 `scripts/verify-production.mjs` 는 두 형식을 모두 인식해야 한다.
